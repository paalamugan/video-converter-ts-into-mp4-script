...
export const getTmpCollectionFilePath = (
  tmpDir: string,
  fileName: string,
  ext: string
) => {
...
export const recursiveRequest = ({
  ...
  return new Promise<number>((initResolve, initReject) => {
    ...
    const makeRequest = (startIndex: number, stopIndex?: number) => {
      ...
      const tmpTsChunkPath = getTmpCollectionFilePath(
        tmpDir,
        `${startIndex}`,
        'ts'
      );
      ...
      request.get(newUrl.href, (response) => {
        ...
        file.on('finish', () => {
          console.log(
            `${CONSOLE_COLOR.cyan}Downloaded(${fileName}): ${
              CONSOLE_COLOR.yellow + startIndex
            }.ts`
          );
          file.close();
        });
...
export const mergeMultipleTsFileToSingle = (
  tmpDir: string,
  fileName: string
) => {
  const segmentOutputFilePath = getTmpCollectionFilePath(
    tmpDir,
    fileName,
    'txt'
  );
